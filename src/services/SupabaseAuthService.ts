...
export class SupabaseAuthService {
    ...
    async syncUserToAuth(
        ...
        try {
            ...
            const { error } = await this.supabase.from('auth.users').upsert(authUserData, { onConflict: 'id' });
    ...
    async updateUserAuth(
        ...
        try {
            ...
            const { error } = await this.supabase.from('auth.users').update(updates).eq('id', authUserId);
    ...
    async getAuthUser(clerkUserId: string): Promise<any> {
        try {
            const { data, error } = await this.supabase.from('auth.users').select('*').eq('id', clerkUserId).single();
