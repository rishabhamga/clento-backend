...
import { PreviewCsvDto, PublishLeadListDto, LeadListResponseDto, LeadListInsertDto, LeadListUpdateDto, LeadInsertDto, LeadListQueryDto } from '../dto/leads.dto';
...
export class LeadListService {
     ...
     */
    async createLeadList(data: LeadListInsertDto, organizationId: string, creatorId: string): Promise<LeadListResponseDto> {
        try {
     ...
     */
    async getLeadLists(organizationId: string, query: LeadListQueryDto): Promise<{ data: LeadListResponseDto[]; pagination: { page: number; limit: number; total: number; totalPages: number } }> {
        try {
     ...
     */
    async getLeadListsWithStats(organizationId: string, query: LeadListQueryDto): Promise<{ data: (LeadListResponseDto & { statistics: any })[]; pagination: { page: number; limit: number; total: number; totalPages: number } }> {
        try {
     ...
     */
    async updateLeadList(leadListId: string, data: LeadListUpdateDto, organizationId: string): Promise<LeadListResponseDto> {
        try {
            ...
            if (disallowedFields.length > 0) {
                throw new BadRequestError(`You are not allowed to update the following fields: ${disallowedFields.join(', ')}. Only 'name' and 'connected_account_id' can be updated.`);
            }
    ...
    async deleteLeadList(leadListId: string, organizationId: string): Promise<void> {
        try {
            ...
            if (leadList.csv_file_url) {
                try {
                    await this.storageService.deleteCsvFile(organizationId, leadListId, `${leadList.name}.csv`, this.bucketName);
                } catch (error) {
    ...
    async publishLeadList(
        ...
        try {
            ...
            try {
                uploadResult = await this.storageService.uploadCsvFile(csvBuffer, filename, organizationId, leadList.id, this.bucketName);
            } catch (error) {
     ...
     * Duplicate lead list
     */
    async duplicateLeadList(leadListId: string, newName: string, organizationId: string, creatorId: string): Promise<LeadListResponseDto> {
        try {
            // Verify lead list exists and belongs to organization
            await this.getLeadListDataById(leadListId, organizationId);

            const duplicatedLeadList = await this.leadListRepository.duplicate(leadListId, newName, organizationId, creatorId);
