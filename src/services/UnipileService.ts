...
export class UnipileService {
  ...
  private initializeClient(): void {
    try {
      ...
      this.client = new UnipileClient(env.UNIPILE_API_URL, env.UNIPILE_API_KEY);
  ...
  isConfigured(): boolean {
    return !!env.UNIPILE_API_KEY && this.client !== null;
  }
  ...
  async sendLinkedInInvitation(params: {
    ...
    try {
      const response = await this.client.users.sendInvitation({
        account_id: params.accountId,
  ...
  async visitLinkedInProfile(params: {
    ...
    try {
      const response = await this.client.users.getProfile({
        account_id: params.accountId,
  ...
  async commentLinkedInPost(params: {
    ...
    try {
      const response = await this.client.users.sendPostComment({
        account_id: params.accountId,
        post_id: params.postId,
        text: params.text,
      });
  ...
  async sendEmail(params: {
    ...
    try {
      const response = await this.client.email.send({
        account_id: params.accountId,
        to: params.to,
