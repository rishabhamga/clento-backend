...
export class CsvService {
    ...
    static validateCsv(parseResult: CsvParseResult): CsvValidationResult {
        ...
        const nameColumn = this.findColumnsByType(headers, 'name')[0] || this.findColumnsByType(headers, 'first_name')[0];
    ...
    static generateMapping(headers: string[]): Record<string, string> {
        ...
        Object.entries(this.COLUMN_MAPPINGS).forEach(([standardField, possibleNames]) => {
            const matchedHeader = headers.find(header => possibleNames.some(name => header.toLowerCase().includes(name.toLowerCase()) || name.toLowerCase().includes(header.toLowerCase())));
     ...
     */
    static applyMapping(data: Record<string, any>[], mapping: Record<string, string>): Record<string, any>[] {
        return data.map(row => {
    ...
    private static findLinkedInColumn(headers: string[]): string | null {
        ...
        return headers.find(header => linkedInPatterns.some(pattern => header.toLowerCase().includes(pattern.toLowerCase()))) || null;
    }
    ...
    private static findColumnsByType(headers: string[], type: keyof typeof CsvService.COLUMN_MAPPINGS): string[] {
        ...
        return headers.filter(header => patterns.some(pattern => header.toLowerCase().includes(pattern.toLowerCase())));
    }
    ...
    private static isValidLinkedInUrl(url: string): boolean {
        try {
            ...
            return urlObj.hostname.includes('linkedin.com') && (urlObj.pathname.includes('/in/') || urlObj.pathname.includes('/company/'));
        } catch {
            return false;
        }
    }
    ...
    static async getPreviewFromUnipile(parseResult: CsvParseResult, maxRows: number = 5, accountId: string) {
        ...
        const publicIdentifiers = leads.map(lead => CsvService.extractLinkedInPublicIdentifier(lead.linkedin_url)).filter(identifier => identifier !== null);
